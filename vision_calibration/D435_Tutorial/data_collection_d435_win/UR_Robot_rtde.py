...
# from real.realsenseD435 import RealsenseD435 as Camera
...
class UR_Robot():
    def __init__(self, tcp_host_ip="192.168.1.33", tcp_port=30003, workspace_limits=None,
                 is_use_robotiq85=None, is_use_camera=None):
        # Init varibles
        if workspace_limits is None:
            workspace_limits = [[-0.7, 0.7], [-0.7, 0.7], [0.00, 0.6]]
        self.workspace_limits = workspace_limits

        # self.rtde_c = rtde_control.RTDEControlInterface(tcp_host_ip)
        self.rtde_r = rtde_receive.RTDEReceiveInterface(tcp_host_ip)
    ...
    def move_j(self, joint_configuration,k_acc=1,k_vel=1,t=0,r=0):
        print(f"movej([{joint_configuration}])")
        self.rtde_c.moveJ(joint_configuration, k_vel, k_acc)
        time.sleep(1.5)
    ...
    def move_j_p(self, tool_configuration,k_acc=1,k_vel=1,t=0,r=0):
        ...
        self.rtde_c.moveJ_IK(tool_configuration, k_vel, k_acc)
    ...
    def move_l(self, tool_configuration,k_acc=1,k_vel=1,t=0,r=0):
        ...
        self.rtde_c.moveL(tool_configuration, k_vel, k_acc)
    ...
    def get_state(self, object):
        if object == 'joint_config':
            joint_config = self.rtde_r.getActualQ()
            return joint_config
        elif object == 'cartesian_pose':
            cartesian_pose = self.rtde_r.getActualTCPPose()
    ...
    def get_current_tool_pos(self):
        return self.get_state("cartesian_pose")
...
if __name__ =="__main__":
    ur_robot = UR_Robot()
    state_data = ur_robot.get_state("cartesian_pose")
    print(state_data)
    ur_robot.rtde_c.disconnect()
    ur_robot.rtde_r.disconnect()
