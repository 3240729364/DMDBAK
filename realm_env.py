...
import math
import time
from pybullet_utils import bullet_client   #如果你想并行使用多个独立的模拟，你可以使用 pybullet_utils.bullet_client
from scipy.spatial.transform import Rotation as R
from loguru import logger
import random
from typing import Optional

class Realm_Env(gym.Env):
    "Custom Environment that follows gym interface."

    metadata = {"rander_models": ["humam"], "render_fps": 30}

    def __init__(self, gui=False):
        super(Realm_Env).__init__()
        self.step_num = 0
        ...
        # 创建机械臂
        self.fr5 = self.p.loadURDF(r"C:\Users\32407\Desktop\reinforcement learning\rm_65_6f_description111\urdf\rm_65_6f_description.urdf", useFixedBase=True,
                                   basePosition=[0, 0, 0],
    ...
    def step(self, action):
        ...
        for i in [1, 2, 3, 4, 5, 6]:
            joint_info = p.getJointState(self.fr5, i-1)  # Fr5中第0个关节是baseline和world的固定关节
        ...
        p.setJointMotorControlArray(self.fr5, [0, 1, 2, 3, 4, 5, 7, 8], p.POSITION_CONTROL, targetPositions=anglenow)
        ...
        for contact_point in target_contact_points:
            link_index = contact_point[3]  # 返回的索引为3的值（第四值）：link index of body A, -1 for base
        ...
        for contact_point in table_contact_points:
            ...
            if link_index == 0:  # 当基坐标连杆与桌子接触时，不算碰撞
                other_contact1 = False
            else:
                other_contact = True
                logger.info("碰撞桌子！")
        ...
        for contact_point in self_contact_points:
            ...
            if (link_indexA == 7 and link_indexB == 8) or link_indexB == 6: # 两个夹爪互相碰撞或者hand碰撞，都不算碰撞
                other_contact1 = False
            else:
                other_contact = True
                logger.info("碰撞自身! ")
        ...
        # 碰撞目标杯子的台子
        for contact_point in self_targettable_contact_points: # 有碰撞点就算碰撞
            other_contact = True
...
# 4.7---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        Gripper_posx = p.getLinkState(self.fr5, 5)[0][0]
        Gripper_posy = p.getLinkState(self.fr5, 5)[0][1]
        Gripper_posz = p.getLinkState(self.fr5, 5)[0][2]
        relative_position = np.array([0, 0, 0.15])
        # 固定夹爪相对于机械臂末端的相对位置转换
        rotation = R.from_quat(p.getLinkState(self.fr5, 6)[1]) # 从四元数返回一个旋转实例
        ...
        if self.step_num == 0:
            ...
            if distance <= 0.41:
                distance_reward = 1000 * (self.distance_last - distance)   #  距离太小的时候，更换计算距离奖励的计算方式
            elif distance > 0.41:
        ...
        # 3.姿态奖励
        # 计算夹爪的朝向
        gripper_orientation = p.getLinkState(self.fr5, 6)[1]
        ...
        obs_gripper_centre_pos = np.array([(gripper_centre_pos[0] + 0.922) / 1.844,       # ?这个计算有些奇怪
                                           (gripper_centre_pos[1] + 0.922) / 1.844,
                                           (gripper_centre_pos[2] + 0.5) / 1], dtype=np.float32)

        obs_gripper_orientation = (np.array([gripper_orientation[0], gripper_orientation[1], gripper_orientation[2]],# ?这个计算有些奇怪
                                            dtype=np.float32) + 180) / 360

        obs_target_position = np.array([(self.target_position[0] - 0.6) / 0.2,   # ?这个计算有些奇怪
                                        (self.target_position[1] + 0.2) / 0.4,
                                        (self.target_position[2] - 0.1) / 0.2], dtype=np.float32)

        joint_angles = [0, 0, 0, 0, 0, 0]
        for i in [1, 2, 3, 4, 5, 6]:
            joint_info = p.getJointState(self.fr5, i-1)
        ...
        self.observation = np.hstack((obs_joint_angles,
                                     obs_gripper_centre_pos,
                                     obs_target_position),   # ?感觉应是obs_gripper_orientation
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        p.setJointMotorControlArray(self.fr5, [0, 1, 2, 3, 4, 5, 7, 8], p.POSITION_CONTROL,   # 忽略关节6（固定关节）
        ...
        # 固定夹爪相对于机械臂末端的相对位置转换
        rotation = R.from_quat(p.getLinkState(self.fr5, 6)[1])
        rotated_relative_position = rotation.apply(relative_position)
    ...
    def get_image(self, width, height):
        return p.getCameraImage(width, height)

if __name__ == "__main__":
    from stable_baselines3.common.env_checker import check_env

    Env = Realm_Env(gui=True)
    Env.reset()
